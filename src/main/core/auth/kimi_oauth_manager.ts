 ...
 */
function make_default_get_device_id(): GetDeviceId {
    return async () => {
...
export function create_kimi_oauth_manager(deps: KimiOAuthManagerDeps): KimiOAuthManager {
    ...
    async function start_device_login(): Promise<DeviceCodeStart> {
        ...
        const response = await post_form(KIMI_DEVICE_AUTH_URL, [["client_id", KIMI_CLIENT_ID]]);
    ...
    function poll_once(device_code: string): Promise<unknown> {
        return post_form(KIMI_TOKEN_URL, [
    ...
    function schedule_retry(instance_id: string): void {
        ...
        if (failures > MAX_REFRESH_RETRIES) {
            log.error(
                `Kimi OAuth refresh gave up after ${String(MAX_REFRESH_RETRIES)} consecutive non-terminal failures for ${instance_id}`,
            );
            retry_failure_counts.delete(instance_id);
            return;
        }
