...
- [x] 新增/更新多账号测试：平均视图、账号明细视图、单账号不显示 L2 segmented。（provider_card.test.tsx 覆盖 L2 segmented 和 count badge）
...
- [ ] 添加数据源弹窗：URL、密钥、同步范围、测试连接、保存并同步。

### 23.3 添加账号流程

- [x] 点击添加进入服务选择 picker：常用服务网格图标 + 高级方式（CPA Manager 入口）。
- [ ] 选择服务后进入对应表单：账号名称、API 密钥（显隐）、接口地址（可选）、测试连接、保存。
...
- [ ] 隐藏后可在设置中重新显示；删除后不可恢复（需确认弹窗）。

### 23.5 账号页布局

- [ ] 单账号厂商一行展示（Logo + 备注名 + 脱敏 key + 操作），不放在分组卡内。
- [ ] 多账号厂商分组展示：厂商头 + 子行列表（拖拽手柄 + 状态点 + 备注名 + key + 操作）。
- [ ] CPA 来源 badge 在行内操作区显示。
