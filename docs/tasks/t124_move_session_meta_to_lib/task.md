...
| -------------- | ------------------------ | ------ | --------- | --------- |
| t124_code_f001 | critical/important/minor | 已修   | {一句话}  | {文件:行} |
...
- [ ] `session_meta` 定义位于 `src/renderer/lib/session_meta.ts`，类型签名不变。
- [ ] `AccountDialog.tsx` 从新路径导入；不再存在 `components` 对 `views/settings-view/lib` 中 `session_meta` 的 import。
- [ ] `settings-view/lib.ts` 不再导出 `session_meta`。
- [ ] typecheck 通过。
- [ ] `pnpm test` 全绿。
- [ ] 行为零变化。
