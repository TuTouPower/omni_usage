...
- [ ] AC1：用户可按住某个厂商 tab 图标拖动到另一个 tab 位置，松开鼠标后 tab 顺序立即更新。
- [ ] AC2：拖拽过程中，被拖 tab 有可视化拖动状态，目标位置有可视化放置指示。
- [ ] AC3：tab 顺序变更后写入配置持久化，应用重启后按保存的顺序渲染。
- [ ] AC4：仅拖拽图标本身触发排序；点击 tab 仍正常切换当前 tab，不会误触发拖拽。
- [ ] AC5：新增未排序过的厂商或清空排序配置时，tab 回退到默认顺序（与当前一致）。
- [ ] AC6：拖拽排序在 web 构建与 Electron 构建中行为一致。
...
- 回退：若 HTML5 drag & drop 与现有 tab 样式/事件冲突，可回退到当前不可拖拽状态，删除排序配置字段即可。
- 风险：配置字段命名与已有 `provider_order` / `accountOrders` 混淆。
- 回退：排序字段命名为 `providerTabOrder`，与现有 `overviewCardOrder` 等字段区分。
