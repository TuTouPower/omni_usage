...
需求定稿 `docs/session_history_window_questions.md`（决策 7），另加面板间互通入口需求。会话历史窗口需要在多处可打开：代理面板「会话明细」行（单击 / 多选批量）、系统托盘 popup、用量面板、代理面板 header 的「到会话历史」按钮。各面板之间可经按钮互相跳转。本 task 实现全部「打开会话历史」的入口与面板间导航按钮。
