...
- 设置页「外观 → 其他」新增开关：「界面脱敏」/ 副文案：「隐藏所有账号备注名（用量面板与设置列表）」

### 影响面

| 表面                                       | 字段                                                 | 脱敏后                             |
| ------------------------------------------ | ---------------------------------------------------- | ---------------------------------- |
| 用量面板 `ProviderAccountRow` `.card-name` | `account.accountLabel`                               | 空 / 占位「—」/ 仅显示厂商（见下） |
| 用量面板 `AccountUsageRow` `.ai-name`      | 同上                                                 | 同上                               |
...
**不隐藏**：`accountId` 默认不在用量面板展示；若有泄露处一并遮。
...
1. 外观打开「界面脱敏」→ 用量面板多账号卡标题无备注，仅厂商级信息可辨。
2. 设置账号列表无 `· 备注`。
