...
review_20260726_054747 采纳项 17、18：web `session.login/refresh` 返回 `{ok,error}` 与契约不符；`usageboard-web.ts` 双重强转掩盖 `connector.catalog`、`config.createInstance`、`settings.openConnectorsDir`、`kimi`、`buildInfo` 等成员缺失与部分返回类型错误。
