<!-- omni_powers: blueprint/specs/connector-cpa -->
...
- **CPA 账号**：菜单"编辑 + 隐藏"。隐藏只写本地 `accountOverrides.hidden`，存在性由远端 CPA-Manager 决定，本地"删"了下次又回来。隐藏后主面板消失、聚合排除、设置页可恢复。
- **直连账号**：菜单"编辑 + 删除"。删除连本地配置带 secret 一起清。

## 展示边界

- **主面板**：无"CPA" tab，CPA 账号并入对应 provider 卡片。
