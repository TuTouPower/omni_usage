<!-- omni_powers: blueprint/specs/platform-services -->

# 平台服务（LocalAPI / NetClient / SessionManager / Logger）
...
- 可选网关 `/v1/<provider>/...` — 转发到 manifest 声明的固定上游域名（白名单），自动记响应头。**暂不实现**（spec 占位，当前代码未提供此能力）。
- `GET /v1/health` — 健康检查。
