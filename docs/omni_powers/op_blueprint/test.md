<!-- omni_powers: blueprint/test -->

# OmniUsage 测试宪章

测试分层、覆盖、Mock 规则、调试入口、打包验证的唯一真相源。
...
| 打包 smoke | `tests/packaged_smoke/` | Playwright + CDP | 验证 `artifacts/win-unpacked/OmniUsage.exe` 启动、渲染、发现内置连接器、popup 高度回归    |
