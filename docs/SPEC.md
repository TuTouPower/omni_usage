...
- **编译产物**：`compiler.ts` 用 esbuild 将连接器 + SDK 编译为单文件 JavaScript，缓存于 user data 目录（按 source SHA-256 失效）
...
- 参数值均为字符串
- `--usageboard-param KEY=value` 仅保留给手动运行和旧连接器兼容；宿主运行时不通过 argv 传 secret

### 3.3a 端点注入

连接器通过环境变量接收端点 URL 和代理配置：
...
- **`OMNI_SOURCE_INSTANCE_ID`**：当前连接器配置的 `instanceId`，单账号连接器用此值写入 `sourceInstanceId` 和 `accountId`

### 3.3b SDK HttpClient

连接器通过 `ctx.http` 发起 HTTP 请求：
...
- 返回 `Result<T, HttpError>`，`failFromHttp(error, contextLabel)` 将错误转为连接器错误输出
...
{
    "success": false,
    "error": { "code": "missing_config", "message": "请在连接器设置中配置 API Key" }
}
```
...
CPA 连接器特性：端点 `{ "default": null }` 必填；参数 `cpa_mgmt_key`（secret）+ 5 个 `monitor_*` boolean 开关；调用 `/v0/management/auth-files` 和 `/v0/management/api-call`；单个账号失败不阻塞其他；Antigravity 三 URL 回退；Gemini 两步请求。
...
| `states/`      | `{userData}/states/`      | 连接器缓存状态      |
...
### 4.3 ConnectorConfiguration schema
...
```typescript
{
    ...
    name: string,            // 显示名
    enabled: boolean,
    executablePath: string,  // 连接器脚本完整路径
    refreshIntervalSeconds: number,  // 60–3600 秒（1–60 分钟）
    parameterValues: Record<string, string>,
    endpointOverrides: Record<string, string>  // 覆盖连接器 metadata 中的 endpoints 默认 URL
}
```
...
- 连接器执行前由 `RefreshService` 从 `secretsStore` 读取并注入 `parameterValues` 副本
- `secretParamKeys` 映射：从连接器 metadata 的 `parameters` 中 `type === "secret"` 构建
...
app.whenReady()
  ...
  → auto-seed: 为新发现的连接器创建默认 PluginConfiguration
...
- `startAll(config)` — 启动所有 enabled 连接器的定时调度
...
```
refresh(instanceId)
  → 从 configStore 查找 PluginConfiguration（连接器配置）
...
- "刷新"按钮 → 触发所有 enabled 连接器刷新
- ProviderCard 列表（支持多账号、多指标进度条）
...
- **账号级操作**：provider 展开后，每个账号行有独立菜单（编辑 / 隐藏或删除）。操作按来源区分：
    ...
    - **直接添加账号**：菜单显示"编辑"+"删除"。删除会移除本地连接器配置及对应 secret。
...
- 保存按钮（带 "已保存" 反馈）
- 复制按钮（duplicate）
- **持久化设置**：开机自启、启动后最小化、刷新间隔、窗口置顶、强调色、主题（浅色/深色/系统）均保存到 config store，重启后恢复
...
- **IPC**：渲染进程通过 `popup:reportContentHeight` 频道发送 `PopupContentHeightReport`（`window.usageboard.popup.report_content_height`）。该频道是单向的 popup 专用通道，**不属于连接器协议**，不会写入 `docs/archive/plugin_contract.md`。
...
获取单个连接器当前快照（`PluginSnapshotDTO`）。

### 7.3 plugin:refresh / plugin:refreshAll

- `plugin:refresh` — 触发指定实例手动刷新
- `plugin:refreshAll` — 触发所有 enabled 连接器刷新
...
- `config:duplicate` — 复制指定连接器配置（含 secret）
...
- **PopupView**：通过 `event.onConfigChange` 监听，收到后刷新连接器列表
...
- `event:stateChange` — Main → Renderer 广播，当连接器状态变更时推送 `instanceId + snapshot`
...
Cookie 认证相关（MiMo 等需要浏览器 Cookie 的连接器）：
...
| 打包 smoke | `tests/packaged_smoke/` | Playwright + CDP | 验证打包产物可启动、渲染正常、内置连接器可发现；托盘显示需人工验收 |
...
无外部依赖。Electron 内置 Node 即为连接器运行时（`process.execPath` + `ELECTRON_RUN_AS_NODE=1`），三平台一致，用户不需要安装任何额外运行时。
...
- 打包产物从 `process.resourcesPath` 加载托盘图标、窗口图标、内置连接器和 SDK；内置连接器和 SDK 启动时做 SHA-256 完整性校验
