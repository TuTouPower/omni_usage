...
function riskProjectedLevel(pct, elapsed) {
    ...
    if (pct >= 95 || projected >= 1.0) return "red";
...
function displayLabel(raw) {
    ...
    return raw.replace(/^(gemini|minimax|gpt|claude|deepseek)-/i, "").replace(/-preview$/i, "");
}
...
function BarRow({ label, full, value, max, idx = 0, reset, elapsed }) {
    ...
    if (style === "capsule") {
        return (
            <div className="bar-row cap">
                <span className="bar-lbl" title={full || label}>
                    {displayLabel(label)}
                </span>
                <div
                    className="cap-track"
             ...
             region — no straddling, no unreadable overflow on balance like 52/100. */}
                    <span className="cap-val" title={valText}>
                        {valText}
                    </span>
                    {!unused && (
                        <span
                            className="cap-val clip"
                            aria-hidden="true"
    ...
    return (
        <div className="bar-row">
            ...
            <span className="bar-pct" title={valText}>
                {valText}
            </span>
            <span className="bar-date">{date}</span>
            <span className="bar-clock">{clock}</span>
...
function UsageCard({
    ...
    const dH5 = limitMode && h5 >= 90,
        dWk = limitMode && week >= 90;
    ...
    return (
        ...
        >
            <div className="card-head">
                ...
                {multi && collapsed && (
                    <span className="count-badge">{!l2open ? "概览" : accts.length + "账号"}</span>
                )}
                {multi && !collapsed && (
                ...
                <div className="card-tools">
                    <button
                        className={"icon-btn card-refresh" + (refreshing ? " spinning" : "")}
                        onClick={onRefresh}
                        title="刷新"
                    >
            ...
            {collapsed ? null : refreshing ? (
                ...
                <div className="card-state err">
                    ...
                    <span>刷新失败 · 网络异常</span>
                    <span className="cs-action" onClick={onRefresh}>
                        重试
                    </span>
                </div>
            ) : state === "auth" ? (
                ...
                <div className="acct-detail">
                    {accts.map((a) => {
                        const adH5 = limitMode && a.h5 >= 90,
                            adWk = limitMode && a.week >= 90;
                        return (
    ...
    return (
        <svg className="chart" viewBox={`0 0 ${W} ${H}`} preserveAspectRatio="none">
            ...
            {yTicks.map((t) => {
                ...
                return (
                    <g key={t}>
                        ...
                        <text className="axis-lbl" x={padL - 7} y={y + 3.5} textAnchor="end">
                            {yLabels[t]}
                        </text>
                    </g>
                );
            ...
            {data.labels.map((lb, i) => {
                ...
                return (
                    <text key={i} className="axis-lbl" x={x} y={H - 7} textAnchor={anchor}>
                        {lb}
                    </text>
                );
            })}
