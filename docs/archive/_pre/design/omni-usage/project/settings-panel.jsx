            ...
            <div className="acct-dialog confirm" onMouseDown={(e) => e.stopPropagation()}>
                ...
                <div className="ad-foot">
                    <div className="ad-foot-r">
                        ...
                        <button className="ad-btn danger" onClick={onConfirm}>
                            删除账号
                        </button>
                    </div>
                </div>
    ...
    return (
        ...
        >
            ...
            <div className="ar-actions">
                {isCpa ? (
                    removed ? (
                        <button className="sub-clear" title="清除该来源已移除的账号">
                            清除
                        </button>
                    ) : (
                    ...
                    <>
                        ...
                        <button className="sp-ic" title="刷新">
                            <Icon name="refresh" size={15} />
                        </button>
                        <button className="sp-ic" title="编辑" onClick={onEdit}>
                            <Icon name="edit" size={15} />
                        </button>
                        <button
                            className="sp-ic danger"
                            title="删除账号"
        ...
        <div className="acc-card">
            {accounts.map((a) => (
                <AccountRow
                    ...
                    onEdit={() => onEditAcct(vendor.id, a.name)}
                />
            ))}
        </div>
    );
        ...
        <div className={"acc-card" + (on ? "" : " off")}>
            <div className="acc-row ds-row">
                ...
                <div className="ar-actions">
                    ...
                    <button className="sp-ic" title="编辑（连接设置）" onClick={onOpenDetail}>
                        <Icon name="edit" size={15} />
                    </button>
                    <button
                        className="sp-ic danger"
            ...
            {conn.accounts.map((a) => (
                <AccountRow
                    ...
                    onHide={() => onHide(a.id)}
                    onEdit={() => onEditAcct(a.vendor, a.name)}
                />
            ))}
            {confirm && (
                <ConfirmDelete
                ...
                <div className="wv-body">
                    <div className="wv-card">
                        <div className="wv-logo">
                            <Icon name="user" size={22} color="var(--text-3)" />
                        </div>
                        <div className="wv-h">登录 {meta.host}</div>
                        <div className="wv-inp" />
                        <div className="wv-inp" />
                        <button className="wv-go" onClick={() => setStage("done")}>
                            登录
                        </button>
                    </div>
                </div>
        ...
        <div className="aa-captured">
            ...
            <div className="cap-text">
                ...
                <div className="cap-sub">
                    来自 {meta.host} · {meta.cookieKeys.join("、")}
                </div>
            </div>
            <button className="cap-redo" onClick={() => setStage("open")}>
                重新登录
            </button>
        </div>
    );
...
function EditAccountDialog({ vendorId, accountName, onClose }) {
    ...
    const seMeta = (window.AUTH_SESSION_META && window.AUTH_SESSION_META[vendorId]) || {
        host: "",
        loginUrl: "",
        cookieKeys: [],
    };
    ...
    const [key, setKey] = React.useState(
        auth === "apikey" ? (akMeta.prefix || "sk-") + "••••••••••••" : "",
    );
    const [showKey, setShowKey] = React.useState(false);
    const [endpoint, setEndpoint] = React.useState("");
    const [useGlobalRefresh, setUseGlobalRefresh] = React.useState(
        !(refreshMeta && refreshMeta.manualDefault),
    );
    const [acctInterval, setAcctInterval] = React.useState(
        refreshMeta && refreshMeta.manualDefault ? "仅手动" : "5 分钟",
    );
    ...
    return (
        <Dialog onClose={onClose}>
            <div className="ad-head">
                ...
                <button className="ad-close" onClick={onClose} title="关闭">
            ...
            <div className="ad-body">
                    ...
                    <>
                        ...
                        <div className="ad-field">
                            ...
                            <input
                                ...
                                placeholder={akMeta.endpoint || "默认（官方接口）"}
                            />
                        </div>
                    </>
                )}
                ...
                <div className="ad-field ad-refresh">
                    <div className="ad-toggle-row">
                        <div className="atr-text">
                            ...
                            <div className="atr-sub">
                                {useGlobalRefresh
                                    ? `当前全局为「${globalInterval}」自动刷新`
                                    : "已为该账号单独设置刷新频率"}
                            </div>
                        </div>
                        <SPToggle
                            on={useGlobalRefresh}
                            onClick={() => setUseGlobalRefresh((v) => !v)}
                        />
                    </div>
                    {!useGlobalRefresh && (
                ...
                {rows.length > 0 && (
                    <div className="ad-field">
                        <label className="ad-label">
                            数据标签映射
                            <span className="ad-opt">{syncOn ? "该数据源共用" : "仅此账号"}</span>
                        </label>
                                    ...
                                    <div className="lm-row" key={r.raw}>
                                        ...
                                        <div className="lm-inwrap">
                                            <input
                                                ...
                                                value={v}
                                                placeholder={r.def}
                                                onChange={(e) => setLbl(r.raw, e.target.value)}
                                            />
                                            {changed && (
                                                <button
                                                    className="lm-reset"
                                                    title="恢复默认"
                                                    onClick={() => resetLbl(r.raw)}
                                                >
                                                    <Icon
                                                        name="reset"
                                                        size={13}
        ...
        <div className="cpa-detail">
            ...
            <div className="cpa-cfg">
                ...
                <div className="cpa-foot">
                    <button className="cf-save">
                        <Icon name="check" size={15} color="#fff" />
                        保存
                    </button>
                    <button className="cf-remove">
            ...
            <div className="cpa-scope">
                ...
                <div className="disc-desc">
                    仅同步已勾选服务商的账号，其用量将显示在主面板。点击编辑可调整该服务商的数据标签映射。
                </div>
                {scopeList.map((id) => (
            ...
            {labelEdit && (
                <LabelMapDialog vendorId={labelEdit} onClose={() => setLabelEdit(null)} />
            )}
        </div>
    );
}
...
function Dialog({ onClose, children, wide }) {
    ...
    return (
        <div className="acct-dialog-scrim" onMouseDown={onClose}>
            <div
                className={"acct-dialog" + (wide ? " wide" : "")}
...
function PickerDialog({ onPickVendor, onPickCpa, onClose }) {
    return (
        <Dialog onClose={onClose}>
            ...
            <div className="ad-body">
                <div className="sp-grouplabel" style={{ marginTop: 0 }}>
                    常用服务
                </div>
                ...
                return (
                    ...
                    >
                        ...
                        <span className="bsf-swatch">
                            {o.swatch.map((c, i) => (
                                <span key={i} className="bsf-dot" style={{ background: c }} />
                            ))}
                        </span>
                    </button>
                );
...
function SettingsPanel({
    ...
    React.useEffect(() => {
        window.__omniLabelSync = s.labelMapSync;
    }, [s.labelMapSync]);
    // expose 全局自动刷新间隔 so the 编辑账号 dialog can show the current value
    React.useEffect(() => {
        window.__omniRefreshInterval = s.interval;
    }, [s.interval]);
    ...
    return (
        <div className="sp-window">
            <div className="sp-nav">
                <div
                    className={"sp-nav-back" + (onBack ? " clickable" : "")}
                    onClick={onBack}
                    role={onBack ? "button" : undefined}
                    title={onBack ? "返回主面板" : undefined}
                >
                    <span className="npc">
                ...
                <div className="sp-nav-items">
                    {nav.map((n) => (
                        <button
                            key={n.id}
                            className={"sp-nav-item" + (section === n.id ? " on" : "")}
            ...
            <div className="sp-main">
                ...
                {inDetail ? (
                    <div className="sp-head">
                        <div className="sp-crumb">
                            <span style={{ cursor: "pointer" }} onClick={() => setDetail(null)}>
                                账号
                            </span>
                            ...
                            <span className="cc-cur">
                                {detailConn ? detailConn.name : "CPA Manager"}
                            </span>
                        </div>
                    </div>
                ) : (
                    <div className="sp-head">
                        ...
                        {section === "accounts" && (
                            <button
                                className="sp-action"
                                onClick={() => setDialog({ type: "picker" })}
                            >
                                <Icon name="plus" size={15} color="#fff" strokeWidth={2.1} />
                                添加
                            </button>
                        )}
                    </div>
                ...
                {inDetail ? (
                    ...
                    <div className="sp-content">
                        {section === "accounts" && (
                            <AccountsPage onOpenDetail={(id) => setDetail(id)} />
                        )}
                        ...
                        {section === "general" && (
                            <>
                                ...
                                <SPRow title="开机时自动启动" sub="登录系统后在后台运行并驻留托盘">
                                    <SPToggle on={true} onClick={() => {}} />
                                </SPRow>
                                <SPRow title="启动后最小化到托盘">
                                    <SPToggle on={true} onClick={() => {}} />
                                </SPRow>
                                ...
                                <SPRow
                                    title="使用代理服务器"
                                    sub="通过代理访问各服务的接口与登录页面"
                                >
                                    <SPToggle
                                        on={s.proxyOn}
                                        onClick={() => up("proxyOn", !s.proxyOn)}
                                    />
                                </SPRow>
                                {s.proxyOn && (
                                ...
                                >
                                    <SPSelect
                                        value={s.panelMode}
                                        onChange={(v) => up("panelMode", v)}
                                        options={["跟随系统推荐", "弹出面板", "浮动窗口"]}
                                    />
                                </SPRow>
                                <SPRow title="窗口始终置顶">
                                ...
                                >
                                    <SPSelect
                                        value={s.floatHeight}
                                        onChange={(v) => up("floatHeight", v)}
                                        options={["保持窗口大小", "跟随内容变化"]}
                                    />
                                </SPRow>
                                ...
                                >
                                    <SPToggle
                                        on={s.labelMapSync}
                                        onClick={() => up("labelMapSync", !s.labelMapSync)}
                                    />
                                </SPRow>
                            </>
                        )}
                                ...
                                >
                                    <div className="set-seg">
                                        {[
                                            ...
                                            <button
                                                key={k}
                                                className={barStyle === k ? "on" : ""}
                                                onClick={() => onBarStyle(k)}
                                            >
                                                {lb}
                                            </button>
                                        ))}
                        ...
                        {section === "notify" && (
                            <>
                                ...
                                <SPRow title="接近限制时提醒" sub="任一周期用量达到 80% 时">
                                    <SPToggle
                                        on={s.notifyNear}
                                        onClick={() => up("notifyNear", !s.notifyNear)}
                                    />
                                </SPRow>
                                <SPRow title="达到限制时提醒" sub="任一周期用量达到 100% 时">
                                    <SPToggle
                                        on={s.notifyLimit}
                                        onClick={() => up("notifyLimit", !s.notifyLimit)}
                                    />
                                </SPRow>
                                <SPRow title="刷新失败时提醒" sub="连续刷新失败或凭证失效时">
                                    <SPToggle
                                        on={s.notifyFail}
                                        onClick={() => up("notifyFail", !s.notifyFail)}
                                    />
                                </SPRow>
            ...
            {dialog && dialog.type === "cpa" && <CpaAddDialog onClose={() => setDialog(null)} />}
        </div>
    );
}
