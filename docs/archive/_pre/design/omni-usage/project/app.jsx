...
function App() {
    ...
    const onResizeDown = (e) => {
        ...
        const move = (ev) =>
            setWinW(Math.max(MIN_W, Math.min(MAX_W, startW + (ev.clientX - startX))));
        const up = () => {
    ...
    const scopeBaseIds =
        tab === "overview"
            ? VENDORS.map((v) => v.id)
            : currentVendor
              ? currentVendor.accounts.map((a) => a.key)
              : [];
    const orderedIds = (order[tab] || scopeBaseIds).filter(
        (id) => scopeBaseIds.includes(id) && !removed.has(tab + ":" + id),
    );
    const missingIds = scopeBaseIds.filter(
        (id) => !orderedIds.includes(id) && !removed.has(tab + ":" + id),
    );
    ...
    const renderCards = () => {
        ...
        if (demo === "error") {
            items.push(
                <div className="net-banner" key="nb">
                    <Icon name="cloud_off" size={18} />
                    <span>网络连接异常，部分数据可能不是最新</span>
                    <span className="nb-action" onClick={refreshAll}>
                        重新连接
                    </span>
                </div>,
            );
            ...
            finalIds.forEach((id) => {
                ...
                items.push(
                    <UsageCard
                        ...
                        name={v.name}
                        accounts={v.accounts}
                        balanceOnly={v.balanceOnly}
                        balance={v.balance}
                        mcp={v.mcp}
                        metrics={d.metrics}
                        l2open={l2set.has(v.id)}
                        onToggleL2={() => toggleSet(setL2set, v.id)}
                        updated={getUpdated(key, v.updated)}
                        h5={d.h5}
                        week={d.week}
                        r5={d.r5}
                        rw={d.rw}
            ...
            finalIds.forEach((id) => {
                ...
                items.push(
                    <UsageCard
                        ...
                        updated={getUpdated(key, a.updated)}
                        h5={d.h5}
                        week={d.week}
                        r5={a.r5}
                        rw={a.rw}
                            ...
                            <>
                                <div className="titlebar" onContextMenu={openCtx}>
                                    ...
                                    <div className="tb-actions">
                                        <button
                                            className={"icon-btn" + (globalSpin ? " spinning" : "")}
                                            title="刷新全部"
                                            onClick={refreshAll}
                                        >
                                ...
                                <div className="tabs-wrap" ref={tabWrapRef}>
                                    <button
                                        className={
                                            "tab pinned" + (tab === "overview" ? " active" : "")
                                        }
                                    ...
                                    <div className="scroll-inner" ref={innerRef}>
                                        ...
                                        {showChart && (
                                            <TokenPanel
                                                chartData={chartData}
                                                totals={totals}
                                                collapsed={tokCollapsed}
                                                onToggleCollapse={() => setTokCollapsed((v) => !v)}
                                                onHandleDown={() => {}}
                                            />
                                        )}
                                    </div>
                    ...
                    {settingsOpen && (
                        <div className="sp-stage">
                            <SettingsPanel
                                ...
                                barScheme={t.barScheme}
                                onBarScheme={(v) => setTweak("barScheme", v)}
                                barStyle={t.barStyle}
                                onBarStyle={(v) => setTweak("barStyle", v)}
