...
| --------------------------------- | -------------------------------------------------------------------------------------- | ---- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------- |
| §3.1 插件文件                     | `.ts` 源 + esbuild 编译 + Electron Node 执行；打包内置插件和 SDK 启动时做 SHA-256 校验 | ⚠️   | `unit/plugin/compiler.test.ts` (compile/cache/error/stale), `unit/plugin/discovery.test.ts` (skip non-.ts, `_` 前缀), `unit/plugin/bundled_resource_verifier.test.ts` (hash mismatch / unexpected file rejection)     | 缺：`_common.ts` 被跳过的显式断言；打包后 `process.resourcesPath/plugins/` 路径解析 |
...
| §3.3 参数传递                     | stdin JSON 传参，非空才传，注入 LANGUAGE，argv 仅兼容手动运行                          | ✅   | `unit/plugin/command-builder.test.ts` (nodePath, stdin 参数格式, argv 不泄露参数值, 跳过空值, LANGUAGE 注入, 中文/特殊字符)                                                                                           | —                                                                                   |
