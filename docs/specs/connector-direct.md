...
| `grok`        | grok        | poll    | 直连 | Grok API（OAuth device-code）；billing 200 但零有效 usage 字段时 connector 须 `report_failed_account`，不得静默 `return []`；已确认 unified billing 且 on-demand cap/used 与 prepaid balance 均为 0 时错误为 `billing account has no available quota or usage data`，不得伪造 0%（t039/t159） |
