...
`TokenStatsView.loadData` 一次拉 buckets（env/source/date filter）+ sessions（env/source）+ records（env/agent/start/end/limit）+ heatmap（env/agent/start/end）+ hour buckets（env/agent/start/end，时间轴 + 小时粒度时，7d/30d 与 24h preset）。records 降为 Bar 项目/会话轴专用辅助，不再作为 KPI/donut/SessionTable/Heatmap 的主数据源。

24h preset 例外（t168）：短窗口（≤25h）下 buckets 日级聚合无法对称切分 current/prev（48h vs 24h），KPI/donut delta 改用 records 驱动（精确 epoch 切分，prevRangeRecords 半开区间）。loadData 短窗口 records 拉 2 倍宽 + limit 50000 覆盖前段。≥7d preset 仍走 buckets（日级误差占比小）。24h preset 的**时间轴小时柱**不走 records（t183，见上表）——高密度时 records 倒序 LIMIT 会截断最早时段（p020）；其 KPI/donut 与项目/会话轴仍走受限 records，由 t184 处理。
