...
- 统一 `AddAccountDialog` 与 `AccountDialog` 的认证区域：两者按同一套规则渲染对应认证区块。
- 新增通用 `DeviceLoginSection`，替代 `OAuthDeviceForm` 与 `GrokLoginSection`，同时支持添加（临时 instanceId）与编辑（真实 instanceId）场景。
- 新增 `WebLoginSection` / `SessionSection`，使 `web_login` / `session` 在添加与编辑侧使用同一组件。
- `SettingsForm` 移除 Grok 特殊 case，改为按 `authMethod` 与 `authDescriptor` 分发到对应登录组件。
