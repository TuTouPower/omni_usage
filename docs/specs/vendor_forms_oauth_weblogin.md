...
- opencode_go：原有 `SessionForm` 把「粘贴 Cookie」作为主路径，「网页登录」只是辅助按钮。用户要求主路径直接触发 `session.login`，不再展示 cookie 输入框。
...
- 抽取 `use_grok_device_login.ts` hook，`OAuthDeviceForm` 与 `GrokLoginSection` 共用设备码状态机。
- `grok_oauth_manager.ts` 在保存 token 后返回 `token`，由 `OAuthDeviceForm` 按 descriptor 的 `secret_name` 写入 `secrets`。
- `grok_oauth_manager.ts` 维护 `active_login_cancels`，支持 `login_cancel`；hook cleanup 调用取消，防止组件卸载后后台轮询继续。
- `use_grok_device_login.start` 入口检查 `active_ref.current`，防止并发轮询覆盖 cancel 句柄。
- `AddAccountDialog` 渲染 `OAuthDeviceForm`/`WebLoginForm` 时加 `key={vendor_id}`，切换服务即卸载重挂。
- `AddAccountDialog` 内子组件拆入 `src/renderer/components/add_account/`，主文件从 609 行降至 308 行。
