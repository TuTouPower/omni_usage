...
3. ~~CPA 仍有展开按钮和 `open_cpa_rows` 状态~~ → 已删除，CPA 子账号始终显示。
4. ~~CPA 子账号行仍显示用量信息~~ → 已移除。
5. CPA 子账号编辑目前打开 `LabelMapDialog`，实际语义是标签映射。最终设计要求"改名/隐藏"，标签映射在编辑面板内。（暂保留 `LabelMapDialog` 作为"改名"入口）
6. ~~`LabelMapDialog` 仍作为独立弹窗存在~~ → 仅保留 CPA 子账号"改名"入口；直连账号编辑面板内已无独立标签映射按钮。
...
5. 部分完成：把标签映射放进编辑面板。
    - ✅ 普通账号列表已无独立标签映射按钮。
    - ✅ CPA 编辑页已移除标签映射按钮。
    - ⬜ 标签映射内嵌到普通账号编辑面板内（暂缓，当前仅 CPA 子账号"改名"入口保留 LabelMapDialog）。
6. ✅ 对齐 CPA 编辑页。
    - 保留双栏：连接配置 / 已发现账号。
    - ✅ 密钥输入补显示/隐藏按钮。
...
8. `pnpm test` 通过。
9. 手工验证：标签切换、滚轮循环、MiMo 失效文案、关于页布局、托盘菜单。
