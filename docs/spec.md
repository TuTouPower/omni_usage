...
- 颜色阈值：>=75% 黄色，>=90% 红色
- **总览页就地展开**（Phase 20 Bug 2 修复）：总览 tab 下的 ProviderCard 支持 chevron 展开/折叠，展开后显示该 provider 的账号列表，折叠/展开驱动 Phase 20 高度自适应。`>` 按钮保留为"跳转到 provider 详情 tab"的辅助入口。
