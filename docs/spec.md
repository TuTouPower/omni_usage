...
- **右键点击**：弹出上下文菜单
    - **设置**：切换到 Settings 视图（同 Popup 窗口内，hash 路由 `#settings`）
    - **退出**：退出应用
...
| Settings | `#settings` | 同 Popup 窗口（460px 内切换）    | 无    | 在 Popup 窗口内通过 hash 路由切换，高度固定 700px                         |
...
- 在 Popup 窗口内通过 `#settings` 路由切换，使用返回按钮回到 `#popup`
