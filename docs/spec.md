...
- **IPC**：渲染进程通过 `popup:reportContentHeight` 频道发送 `PopupContentHeightReport`（`window.usageboard.popup.report_content_height`）。该频道是单向的 popup 专用通道，**不属于插件协议**，不会写入 `docs/plugin_contract.md`。
...
Main 进程在 config save 成功后，通过 `CONFIG_CHANGED` IPC 向所有窗口广播新配置。各 renderer 窗口监听该事件以保持配置同步：
...
- **自身 save 去重**：`save()` 乐观更新本地 state 并保存 `config_ref` 引用；收到 `CONFIG_CHANGED` 时若引用相同则跳过，避免重复渲染
