...
| 打包 smoke     | `tests/packaged_smoke/` | Playwright + CDP | 验证 `artifacts/win-unpacked/OmniUsage.exe` 能正常启动、渲染、发现内置插件并覆盖 popup 窗口高度回归 |
...
- `artifacts/win-unpacked/OmniUsage.exe` 首次启动。
