...
**Tier 2:沙箱脚本(能力注入)。** 响应需要编排或解析逻辑时(CPA 多账号多步、CPA Antigravity 模型配额分组、Gemini 两步请求、解析 `~/.claude` 文件格式),manifest 声明 `script: "connector.ts"`。脚本经 esbuild 编译(按源码 SHA-256 缓存,沿用现有机制),在**隔离沙箱**中执行:
