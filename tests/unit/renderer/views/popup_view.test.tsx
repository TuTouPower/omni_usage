...
describe("PopupView", () => {
    ...
    it("overview re-login banner for a single failed connector opens settings with that instanceId", async () => {
        ...
        // Auth-error banner lives inside the collapsible card body.
        // Expand first so the banner is visible.
    ...
    it("overview banner re-login routes to first failed instance when multiple share provider", async () => {
        ...
        // Expand the GroK provider card so the auth banner is visible.
        const expand = await screen.findByRole("button", { name: "展开" });
        fireEvent.click(expand);
