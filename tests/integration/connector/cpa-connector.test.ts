...
describe("cpa connector", () => {
    ...
    it("shows Antigravity model usage from CPA usage history", async () => {
        const script = await readFile(join("connectors", "cpa", "connector.ts"), "utf8");
        const ctx = create_ctx();
        ctx.http.get_json = (_ep, path, opts) => {
            expect(opts?.headers?.["Authorization"]).toBe("Bearer management-key");
            if (path === "/v0/management/auth-files") {
                return Promise.resolve({
                    files: [
                        {
                            name: "auth-antigravity-user@example.com.json",
        ...
        ctx.http.post_json = () => {
            throw new Error("Antigravity quota API should not be called");
        };
        ...
        expect(result.error).toBeNull();
        const antigravity = result.observations.filter((o) => o.provider === "antigravity");
        expect(antigravity).toHaveLength(2);
        expect(antigravity.map((o) => o.raw_label)).toEqual(["gemini-3-pro", "gemini-3-flash"]);
        expect(antigravity[0]).toEqual(
            expect.objectContaining({
                source_instance_id: "cpa",
                account_id: "ag-auth",
                account_label: "antigravity-user@example.com",
                used: 600,
                limit: 600,
                display_style: "ratio",
                source: "gateway",
        ...
        expect(antigravity[1]?.used).toBe(300);
        expect(antigravity[1]?.limit).toBe(600);
    });

    it("ignores Antigravity usage details without matching auth index", async () => {
        const script = await readFile(join("connectors", "cpa", "connector.ts"), "utf8");
        const ctx = create_ctx();
        ctx.http.get_json = (_ep, path) => {
            if (path === "/v0/management/auth-files") {
                return Promise.resolve({
                    files: [
                        {
        ...
        const antigravity = result.observations.filter((o) => o.provider === "antigravity");
        expect(antigravity).toHaveLength(1);
        expect(antigravity[0]?.used).toBe(600);
    });

    it("fetches Antigravity usage history once for multiple accounts", async () => {
        const script = await readFile(join("connectors", "cpa", "connector.ts"), "utf8");
        const ctx = create_ctx();
        let usage_calls = 0;
        ctx.http.get_json = (_ep, path) => {
            if (path === "/v0/management/auth-files") {
                return Promise.resolve({
                    files: [
                        {
                            name: "auth-antigravity-a@example.com.json",
                            provider: "antigravity",
                            auth_index: "ag-a",
                        },
                        {
                            name: "auth-antigravity-b@example.com.json",
                            provider: "antigravity",
                            auth_index: "ag-b",
                        },
                    ],
                    ...
                    apis: {
                        api: {
                            models: {
                                "gemini-3-pro": {
                                    details: [
                                        { auth_index: "ag-a", tokens: { total_tokens: 600 } },
                                        { auth_index: "ag-b", tokens: { total_tokens: 300 } },
                                    ],
                                },
                            },
                        },
        ...
        expect(result.error).toBeNull();
        expect(usage_calls).toBe(1);
        expect(result.observations.filter((o) => o.provider === "antigravity")).toHaveLength(2);
    });
